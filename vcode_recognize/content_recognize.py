    ...
    def __get_query_content__(self, query_url):
        ...
        if response.status_code == requests.codes.ok:
            ...
            if len(li) == 0:
                ...
                resp = requests.get(once_url, headers=self.headers, verify=False)
                js = resp.json()
                #print("查找相似图片")
                for fromTitle in js['data']:
                    li.append(fromTitle['fromPageTitle'])
                    print(fromTitle['fromPageTitle'])
            return "|".join(x for x in li)

    def get_pic_content(self, img_url):
        box = (0, 41, 295, 186)
...
if __name__ == "__main__":
    img_url = "../img/code.png"
    c = RecoginitionContainer("../img")
    c.get_text(img_url)
